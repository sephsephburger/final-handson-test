...
- Why documents: the same patterns used by Amazon ("Working Backwards") and Google ("Design Docs") to align teams and reduce risk.
- Trinity summary: PRD (scope and truth) -> TODO (roadmap) -> AGENTS (rules), with the agent reading these files before writing code.
- Demo angle: show how the live repo maps to the framework - open PRD/TODO/AGENTS, then run `python main.py` and walk through the timer, settings, tasks, and alerts.

## Next steps
- Add lint/format (`ruff`, `black`) and tests (`pytest`) covering timer transitions, persistence, notifications, task auto-increment, and daily reset logic.
- Decide UX defaults (task list visibility, alarm tone/volume, and whether task auto-increment is enabled by default).
- Capture future findings (e.g., browser autoplay behavior) back into PRD/TODO/AGENTS to keep the AI and contributors aligned.
